...
class ReadL2Data(ReadL2DataBase):
    ...
    def __init__(self, data_id=None, index_pointer=0, loglevel=logging.INFO, verbose=False):
        ...
        self.NAN_DICT.update({self._UPPERALTITUDENAME: -1.0})
        self.NAN_DICT.update({self._RAYLEIGHLATNAME: -1.0e-6})
        self.NAN_DICT.update({self._RAYLEIGHLONNAME: -1.0e-6})
        self.NAN_DICT.update({self._RAYLEIGHALTITUDENAME: -1.0})
        self.NAN_DICT.update({self._EC355NAME: -1.0e6})
        self.NAN_DICT.update({self._BS355NAME: -1.0e6})
        self.NAN_DICT.update({self._LODNAME: -1.0})
        ...
        self.EMEP_VARS_TO_COPY_FOR_GRIDDING = [
            self.EMEP_VAR_NAME_DICT[self._LONGITUDENAME],
    ...
    def read_file(
        ...
        for retrieval in self.SUPPORTED_RETRIEVALS:
            ...
            coda_groups_to_read = self.CODA_READ_PARAMETERS[retrieval]["metadata"][
                self._TIME_NAME
            ].split(self.GROUP_DELIMITER)

            file_data[self._TIME_NAME] = coda.fetch(
                product, coda_groups_to_read[0], -1, coda_groups_to_read[1]
            )
            ...
            file_data[self._TIME_NAME] = (
                ((file_data[self._TIME_NAME] + seconds_to_add) * 1.0e3)
                .astype(np.int)
            ...
            for var in vars_to_read_in:
                ...
                try:
                    ...
                    groups = self.CODA_READ_PARAMETERS[retrieval]["metadata"][var].split(
                        self.GROUP_DELIMITER
                    )
                ...
                if group_depth == 3:
                    for idx, key in enumerate(file_data[self._TIME_NAME]):
                        file_data[var][key] = coda.fetch(
                            product, groups[0], idx, groups[1], -1, groups[2]
                        )

                elif group_depth == 4:
                    for idx, key in enumerate(file_data[self._TIME_NAME]):
                        file_data[var][key] = coda.fetch(
                            product, groups[0], idx, groups[1], 0, groups[2], -1, groups[3]
                        )

                elif group_depth == 2:
                    for idx, key in enumerate(file_data[self._TIME_NAME]):
                        file_data[var][key] = coda.fetch(product, groups[0], -1, groups[1])
                elif group_depth == 1:
                    for idx, key in enumerate(file_data[self._TIME_NAME]):
                ...
                if var == self._QANAME:
                    ...
                    for idx, key in enumerate(file_data[self._TIME_NAME]):
                        ...
                        file_data[var + "_anded"][key] = np.bitwise_and(
                            file_data[var][key], self._QAINDEX_VAL
                        )
            ...
            if return_as == "numpy":
                ...
                for idx, _time in enumerate(file_data["time"].astype(np.float64)):
                    ...
                    if np.sum(np.isin(times_to_skip, file_data["time"][idx])) > 0:
                        self.logger.info(
                            "skipped time {} due to no valid measurement".format(
                                _time.astype("datetime64[ms]")
                            )
                        )
                        self._point_no_found += len(
                            file_data[self._QANAME][file_data[self._TIME_NAME][idx]]
                        )
                        continue
                    else:
                        pass
                    ...
                    _time = _time / 1000.0
                    for _index in range(
                        len(file_data[self._QANAME][file_data[self._TIME_NAME][idx]])
                    ):
                        ...
                        if (
                            file_data[self._QANAME + "_anded"][file_data[self._TIME_NAME][idx]][
                                _index
                            ]
                            == 0
                        ):
                        ...
                        for var in vars_to_read_in:
                            ...
                            if var == self._TIME_NAME or var == "test_sca_pcd_mid_bins_qc_flag":
                                continue
                                # var == self._QANAME or \
                            # longitudes are 0 based for Aeolus, but -180 based for model data
                            # adjust Aeolus to model data
                            if var == self._LONGITUDENAME:
                                data[index_pointer, self.INDEX_DICT[var]] = file_data[var][
                                    file_data[self._TIME_NAME][idx]
                                ][_index]
                                if file_data[var][file_data[self._TIME_NAME][idx]][_index] > 180.0:
    ...
    def calc_dist_in_data(
        self, ungridded_data_obj, location=(49.093, 8.428, 0.0), backend="pyaerocom"
    ):
        ...
        if backend == "pyaerocom":
            if self.rads_in_array_flag:
                ...
                np.deg2rad(
                    ungridded_data_obj._data[:, self._LATINDEX],
                    out=ungridded_data_obj._data[:, self._RADLATINDEX],
                )
                np.deg2rad(
                    ungridded_data_obj._data[:, self._LONINDEX],
                    out=ungridded_data_obj._data[:, self._RADLONINDEX],
                )
            ...
            data[:, self._DISTINDEX] = (
                self.EARTH_RADIUS
                * 2.0
                * np.arcsin(
                    np.sqrt(
                        np.power(np.sin((data[:, self._RADLATINDEX] - lat2) / 2.0), 2)
                        + np.cos(ungridded_data_obj._data[:, self._RADLATINDEX])
                        * np.cos(lat2)
                        * np.power(np.sin((data[:, self._RADLONINDEX] - lon2) / 2), 2)
                    )
                )
            )
            ...
            for idx in range(len(data[:, self._TIMEINDEX])):
                ...
                if np.isnan(
                    ...
                    + ungridded_data_obj._data[idx, self._LONINDEX]
                ):
                ...
                ungridded_data_obj._data[idx, self._DISTINDEX] = geopy.distance.great_circle(
                    location,
                    (
                        ungridded_data_obj._data[idx, self._LATINDEX],
                        ungridded_data_obj._data[idx, self._LONINDEX],
    ...
    ###################################################################################
    def colocate(
        self, ungridded_data_obj, location=None, max_dist=50.0, bbox=None, resample_to_grid=None
    ):
        ...
        if location is not None:
            if isinstance(location, list):
                ...
                for idx_lat, loc in enumerate(location):
                    ...
                    matching_indexes = np.where(
                        ungridded_data_obj._data[:, self._DISTINDEX] < max_dist
                    )
            ...
            unique_times = np.unique(
                ungridded_data_obj._data[matching_indexes, self._TIMEINDEX]
            ).astype("datetime64[s]")
            ...
            grid_lats_end = list(
                np.arange(start_lat + lat_spacing, end_lat + lat_spacing, lat_spacing)
            )
            grid_lons_start = list(np.arange(start_lon, end_lon, lon_spacing))
            grid_lons_end = list(
                np.arange(start_lon + lon_spacing, end_lon + lon_spacing, lon_spacing)
            )
            ...
            for idx_lat in range(len(grid_lats_start)):
                for idx_lon in range(len(grid_lons_start)):
                    bbox_temp.append(
                        (
                            grid_lats_start[idx_lat],
                            grid_lats_end[idx_lat],
                            grid_lons_start[idx_lon],
                            grid_lons_end[idx_lon],
                        )
                    )
            ret_data = {}
    ...
    def read_data_fields(
        self,
        filename,
        coda_handle=None,
        fields_to_read=["mph", "sca_optical_properties"],
        loglevel=None,
    ):
            ...
            if root_field_name in fields_to_read:
                if root_field_name == "mph":
                    ...
                    for fieldname in mph_fieldnames:
                        print(fieldname)
                        try:
                            ret_data[root_field_name][fieldname] = coda.fetch(
                                coda_handle, root_field_name, fieldname
                            )
                    ...
                    if isinstance(coda_data[root_field_name], coda.codapython.Record):
                        ret_data[root_field_name] = self.codarecord2pythonstruct(
                            coda_data[root_field_name]
                        )
                    else:
                        ret_data[root_field_name] = []
        ...
        for idx in range(len(codaRec)):
            if isinstance(codaRec[idx], np.ndarray):
                ...
                for idx2 in range(rec_length):
                    ...
                    for str_name in dummy:
                        out_struct[codaRec._registeredFields[idx]][str_name][idx2] = dummy[
                            str_name
                        ]
            else:
    ...
    def plot_location_map(
        ...
        if plot_time:
            ...
            min_index = np.int(np.floor_divide(len(lats), 10.0))
            ...
            plt.annotate(
                time_str, xy=(x, y), xycoords="data", color="k", fontsize="xx-small", ha="center"
            )
        ...
        for var in vars_to_plot:
            ...
            for time_step_idx, unique_time in enumerate(unique_times):
                ...
                if nansum > 0 and len(var_data) > 1:
                    height_data = _data[
                        time_index_dict[unique_time], self.INDEX_DICT[self._ALTITUDENAME]
                    ]
                    ...
                    if len(height_data) < 10.0:
                        try:
                            lower_height_data = _data[
                                time_index_dict[unique_time],
                                self.INDEX_DICT[self._UPPERALTITUDENAME],
                            ]
                        except TypeError:
                            pass
                        ...
                        for height_level in range(len(height_data)):
                            ...
                            if zero_filled_flag:
                                zero_filled_flag = False
                                plot_heights_temp[temp_height_index] = (
                                    height_data[height_level] + height_step_size
                                )
                            ...
                            if lower_height_data[height_level] in height_data:
                                plot_heights_temp[temp_height_index] = (
                                    lower_height_data[height_level] + height_step_size / 10.0
                                )
                                plot_val_temp[temp_height_index] = var_data[height_level]
                                temp_height_index += 1
                            else:
            ...
            yticklabels = plot_simple2.axes.set_yticklabels(yticks / 100.0)
            ...
            plt.annotate(
                "start={}".format(unique_times[0].astype("datetime64[s]")),
                (0.05, 0.27),
            ...
            plt.annotate(
                "end={}".format(unique_times[-1].astype("datetime64[s]")),
                (0.98, 0.27),
                xycoords="figure fraction",
                fontsize=9,
    ...
    def to_grid(
        ...
        grid_heights=None,
        grid_heights_low=None,
        grid_heights_high=None,
        grid_field=None,
        levelno=20,
    ):
        ...
        if grid_heights is not None:
            ...
            if grid_heights_low is None:
                grid_heights_low = np.arange(levelno + 1) * 1000.0
        ...
        if engine == "python":
            grid_heights = (
                grid_heights_low[:-1] + (grid_heights_low[1:] - grid_heights_low[:-1]) / 2
            )
            ...
            gridded_var_data[self._ALTBOUNDSNAME] = np.transpose(
                np.array(
                    [
                        grid_heights_low[0 : len(grid_heights)],
                        grid_heights_high[0 : len(grid_heights)],
                    ]
                )
            )
            ...
            for height_idx, grid_height in enumerate(grid_heights_low):
                ...
                height_match_indexes = np.where(
                    (diff_height <= max_allowed_height_dist) & (diff_height > 0.0)
                )[0]
                ...
                for lat_idx, grid_lat in enumerate(grid_lats):
                    ...
                    for lon_idx, grid_lon in enumerate(grid_lons):
                        ...
                        lon_match_indexes = np.where(
                            (
                                _data[match_indexes, self._LONINDEX]
                                > (grid_lon - grid_dist_lon / 2.0)
                            )
                            & (
                                _data[match_indexes, self._LONINDEX]
                                <= (grid_lon + grid_dist_lon / 2.0)
                        ...
                        for var in vars:
                            less_than_zero_indexes = np.where(
                                _data[match_indexes, self.INDEX_DICT[var]] > 0.0
                            )[0]
                            ...
                            if return_data_for_gridding:
                                data_for_gridding[var][grid_lat][grid_lon][height_idx] = np.array(
        ...
        if levelno is None or levelno == 1 or levelno == 0:
            ...
            if gridtype in self.SUPPORTED_GRIDS:
                ...
                grid_array_prot = np.full(
                    (
                        self.SUPPORTED_GRIDS[gridtype]["grid_lats"].size,
    ...
    def to_model_grid(
        ...
        if engine == "python":
            ...
            if return_data_for_gridding:
                gridded_var_data_temp, gridded_var_data_temp = self._to_grid_grid_init(
                    vars=vars,
                    levelno=model_levelno,
                    latitudes=grid_lats,
                    longitudes=grid_lons,
                    times=model_times,
                    return_gridding_data_struct=True,
                )
            else:
                gridded_var_data_temp = self._to_grid_grid_init(
                    vars=vars,
                    levelno=model_levelno,
                    latitudes=grid_lats,
                    longitudes=grid_lons,
            ...
            for time_idx, _time in enumerate(model_times):
                ...
                if lat_verbose_flag:
                    print(
                        "model time: {}, matched indexes: {}".format(
                            _time.astype("datetime64[h]"), time_match_indexes.size
                        )
                    )
                diff_time = None
                ...
                for lat_idx, grid_lat in enumerate(grid_lats):
                    ...
                    lat_min = grid_lat - grid_dist_lat / 2.0
                    lat_max = grid_lat + grid_dist_lat / 2.0
                    lat_match_indexes = np.where(
                        (_data[time_match_indexes, self._LATINDEX] > lat_min)
                        & (_data[time_match_indexes, self._LATINDEX] <= lat_max)
                    )[0]
                    # diff_lat = None
                    if lat_verbose_flag:
                        print(f"lat: {grid_lat}, matched indexes: {lat_match_indexes.size}")
                    ...
                    for lon_idx, grid_lon in enumerate(grid_lons):
                        # diff_lon = np.absolute((_data[match_indexes, self._LONINDEX] - grid_lon))
                        # lon_match_indexes = np.where(diff_lon <= (grid_dist_lon / 2.))[0]
                        lon_min = grid_lon - grid_dist_lon / 2.0
                        lon_max = grid_lon + grid_dist_lon / 2.0
                        lon_match_indexes = np.where(
                            (_data[lat_match_indexes, self._LONINDEX] > lon_min)
                            & (_data[lat_match_indexes, self._LONINDEX] <= lon_max)
                        )[0]
                        # diff_lon = None
                        ...
                        model_height = model_data[
                            self.EMEP_VAR_NAME_DICT[self._ALTITUDENAME]
                        ].data[time_idx, :, lat_idx, lon_idx]
                        mean_idx = {}

                        for match_idx, _height in enumerate(
                            _data[lon_match_indexes, self._ALTITUDEINDEX]
                        ):
                            diff_height_to_model = np.absolute(model_height - _height)
                            lowest_idx = np.where(
                                diff_height_to_model == np.amin(diff_height_to_model)
                            )[0]
                            if lat_verbose_flag:
                                print(
                        ...
                        for idx in mean_idx:
                            ...
                            for var in vars:
                                greater_than_zero_indexes = np.where(
                                    _data[data_indexes, self.INDEX_DICT[var]] > 0.0
                                )[0]
                                ...
                                if greater_than_zero_indexes.size < self.MIN_VAL_NO_FOR_GRIDDING:
                                    continue
                                greater_than_zero_indexes = lon_match_indexes[
                                    greater_than_zero_indexes
                                ]
                                # if return_data_for_gridding:
                                ...
                                if greater_than_zero_indexes.size == 1:
                                    ...
                                    try:
                                        ...
                                        matching_points = (
                                            matching_points + greater_than_zero_indexes.size
                                        )
                                        if lat_verbose_flag:
                                            print(
                                                "val: {}".format(
