...
def make_config(
    ...
    cfg["obs_cfg"]["EEA"]["read_opts_ungridded"]["files"] = [  # type:ignore[index]
        str(p)
        for p in obs_paths(
            *obs_dates, root_path=obs_path, analysis=run_type == RunType.AN, force_analysis_obsset=force_analysis_obsset
        )
    ]

    if (run_type == RunType.AN or force_analysis_obsset):
...
def main(
    ...
    force_analysis_obsset: bool = typer.Option(False, "--force_analysis_obsset", help="Meant to be used in combination with eval_type forecast: the observations set will be the one for the analysis, evaluation will be limited to just 1 forecast day. This is a hack to produce plots needed for the quarterly reports."),
