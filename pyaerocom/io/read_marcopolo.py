...
class ReadMarcoPolo(ReadUngriddedBase):
    ...
    FILE_COLS = {"station_id": 0, "datetime": 1, "var_name": 2, "value": 3}
    ...
    def _read_files(self, files, vars_to_retrieve):
        ...
        for _file in tqdm(files, disable=const.QUIET):
            filedata = self._read_file(_file)
            arr = filedata.values
        ...
        for var in vars_to_retrieve:
            ...
            var_stats = self._make_station_data(
                var, var_stats_unique, stat_ids, var_stats, var_dtime, var_values, stat_meta
            )
