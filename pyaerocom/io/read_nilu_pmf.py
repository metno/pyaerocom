...
class LevoFile:
    ...
    def _read_data(self, data):
        ...
        s = data.shape
        new_data = np.zeros((s[0], 1))
        flag_data = np.zeros((s[0], 1), dtype=np.int16)

        self.meta_data["missing_val"] = np.array(
            [self.meta_data["missing_val"][self.data["header"].index(i)] for i in ["levoglucosan"]]
        )
        for i, d in enumerate(data):

            new_data[i, 0] = d[self.data["header"].index("levoglucosan")]
            flag_data[i, 0] = d[self.data["header"].index("levoglucosan_flag")] * 100
        ...
        self.data["header"] = ["levoglucosan"]

        new_time = pd.date_range(time[0], time[-1], freq="d")

        new_nan_data = np.ones((len(new_time), 1)) * np.nan
