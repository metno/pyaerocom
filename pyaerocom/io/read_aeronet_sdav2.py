...
This module has not yet been translated / shipped to the pyaerocom library
"""
...
class ReadAeronetSDAV2(object):
    ...
    DATASET_PATH = const.OBSCONFIG[const.AERONET_SUN_V2L2_SDA_DAILY_NAME][
        'PATH']
    ...
    def read_daily_file(in_file, verbose=False):
        ...
        with open(in_file, 'rt') as in_file:
            ...
            for line in in_file:
                ...
                d_data_out['od500aer'].append(
                    np.float_(c_dummy_arr[TOTAL_AOD_500NM_TAU_A_INDEX]))
                if d_data_out['od500aer'][-1] == f_nan_val:
                    d_data_out['od500aer'][-1] = np.nan
                d_data_out['od440aer'].append(
                    np.float_(c_dummy_arr[I440NM_INPUT_AOD_INDEX]))
                if d_data_out['od440aer'][-1] == f_nan_val:
                    d_data_out['od440aer'][-1] = np.nan
                d_data_out['od870aer'].append(
                    np.float_(c_dummy_arr[I870NM_INPUT_AOD_INDEX]))
                if d_data_out['od870aer'][-1] == f_nan_val:
                    d_data_out['od870aer'][-1] = np.nan

                d_data_out['od500gt1aer'].append(
                    np.float_(c_dummy_arr[COARSE_MODE_AOD_500NM_TAU_C_INDEX]))
                if d_data_out['od500gt1aer'][-1] == f_nan_val:
                    d_data_out['od500gt1aer'][-1] = np.nan
                d_data_out['od500lt1aer'].append(
                    np.float_(c_dummy_arr[FINE_MODE_AOD_500NM_TAU_F_INDEX]))
                if d_data_out['od500lt1aer'][-1] == f_nan_val:
                    d_data_out['od500lt1aer'][-1] = np.nan

                d_data_out['ang4487aer'].append(-1.0 * np.log(
                    d_data_out['od440aer'][-1] / d_data_out['od870aer'][
                        -1]) / np.log(0.44 / .870))
                d_data_out['od550aer'].append(
                    d_data_out['od500aer'][-1] * (0.55 / 0.50) ** np.float_(
                        -1.) * d_data_out['ang4487aer'][-1])
                d_data_out['od550gt1aer'].append(
                    d_data_out['od500gt1aer'][-1] * (0.55 / 0.50) ** np.float_(
                        -1.) * d_data_out['ang4487aer'][-1])
                d_data_out['od550lt1aer'].append(
                    d_data_out['od500lt1aer'][-1] * (0.55 / 0.50) ** np.float_(
                        -1.) * d_data_out['ang4487aer'][-1])
        ...
        d_data_out['od500gt1aer'] = pd.Series(d_data_out['od500gt1aer'],
                                              index=d_Time)
        d_data_out['od500lt1aer'] = pd.Series(d_data_out['od500lt1aer'],
                                              index=d_Time)
        ...
        d_data_out['ang4487aer'] = pd.Series(d_data_out['ang4487aer'],
                                             index=d_Time)
        d_data_out['od550aer'] = pd.Series(d_data_out['od550aer'], index=d_Time)
        d_data_out['od550gt1aer'] = pd.Series(d_data_out['od550gt1aer'],
                                              index=d_Time)
        d_data_out['od550lt1aer'] = pd.Series(d_data_out['od550lt1aer'],
                                              index=d_Time)
    ...
    def get_file_list(self):
        ...
        self.files = glob.glob(os.path.join(ReadAeronetSDAV2.DATASET_PATH,
                                            ReadAeronetSDAV2.FILEMASK))
...
if __name__ == "__main__":
    ...
    p = '/lustre/storeA/project/aerocom/aerocom1/AEROCOM_OBSDATA/AeronetSun2.0.SDA.daily/renamed/'
