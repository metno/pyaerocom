...
@pytest.mark.parametrize(
    "freq,region_ids,use_weights,use_country,data_freq,nmb_avg,raises",
    [
        (
            "yearly",
            {"EUROPE": "Europe"},
        ...
        (
            "monthly",
            {"SEA": "SE Asia(HTAP)", "EUR": "Europe(HTAP)"},
            False,
            False,
            None,
            0.181,
...
def test__process_statistics_timeseries(
    example_coldata, freq, region_ids, use_weights, use_country, data_freq, nmb_avg, raises
):
    with raises:
        result = mod._process_statistics_timeseries(
            example_coldata, freq, region_ids, use_weights, use_country, data_freq
        )
...
@pytest.mark.parametrize(
    "freq, season, start, stop, min_yrs, station, raises",
    [
...
def test__make_trends(coldata, freq, season, start, stop, min_yrs, station, raises):
    with raises:
