...
@pytest.mark.parametrize(
    ...
    [("station_name", 1), ("closest", 0.1), ("closest", 1), ("closest", 30)],
)
def test__map_same_stations_samedata(stats_sun_aod, match_stats_how, match_stats_tol_km):

    (_index_short, _index_long, _statnames_short, _statnames_long) = testmod._map_same_stations(
...
def test__combine_2_sites_same_site(
    stats_sun_aod, merge_how, merge_eval_fun, var_name_out, data_id_out, var_unit_out
):
