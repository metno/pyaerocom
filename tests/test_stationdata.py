...
@pytest.mark.parametrize(
    "stat,force_single_value,dtype,raises",
    [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (stat1, True, True, False, ["random_key1", "random_key2"], 16, does_not_raise_exception()),
...
def test_StationData_get_meta(
    ...
    with raises:
        meta = stat.copy().get_meta(
            force_single_value, quality_check, add_none_vals, add_meta_keys
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (stat1, stat2, 50, True, False, ["random_key1"], True, does_not_raise_exception()),
    ],
)
def test_StationData_merge_meta_same_station(
    ...
    with raises:
        stat = stat.copy()
        val = stat.merge_meta_same_station(
            other.copy(), coord_tol_km, check_coords, inplace, add_meta_keys, raise_on_error
        )
...
@pytest.mark.parametrize(
    "stat,var_name,low,high,check_unit,mean,raises",
    [
...
@pytest.mark.parametrize(
    "clim_freq,avg,raises",
    [
        (None, 0.44, does_not_raise_exception()),
        ("hourly", 0.44, does_not_raise_exception()),
    ],
)
def test_StationData_calc_climatology(aeronetsunv3lev2_subset, clim_freq, avg, raises):
...
@pytest.mark.parametrize(
    "stat,var_name,altitudes,raises",
    [
...
def test_StationData_select_altitude_Series(stat, var_name, altitudes, raises):
    stat = stat.copy()
    with raises:
...
@pytest.mark.parametrize(
    "stat,var_name,kwargs,dtype,raises",
    [
        (stat1, "od550aer", dict(), pd.Series, does_not_raise_exception()),
        (ec_earlinet, "ec532aer", dict(), None, pytest.raises(ValueError)),
        (ec_earlinet, "ec532aer", dict(altitude=(0, 1000)), pd.Series, does_not_raise_exception()),
...
@pytest.mark.parametrize(
    "stat,args,raises", [(stat1, dict(var_name="od550aer"), does_not_raise_exception())]
)
