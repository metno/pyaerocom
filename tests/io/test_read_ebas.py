...
class TestReadEbas(object):
    ...
    AUX_FUNS = {
        "sc440dryaer": pyaerocom.aux_var_helpers.compute_sc440dryaer,
    ...
    def test__get_var_cols(self, reader, loaded_nasa_ames_example, var, raises, colnums):
        vi = EbasVarInfo(var)
        with raises:
    ...
    def test__flag_incorrect_frequencies(
        self, reader, loaded_nasa_ames_example, ts_type, tol_percent, num_flagged
    ):
        st = StationData()
    ...
    def test_read_file(self, reader, filename, vars_to_retrieve, raises, check_attrs):
        with raises:
