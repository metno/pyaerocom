...
def test_ReadCmipCtm__init__(data_dir: str):
    reader = ReadCmipCtm("EMEP_2017", data_dir)
...
def test_ReadCmipCtm__compute_var(var_name, ts_type, data_dir: str):
    reader = ReadCmipCtm(data_dir=data_dir)
    data, proj_info = reader._compute_var(var_name, ts_type)
    assert isinstance(data, xr.DataArray)


def test_ReadCmipCtm__compute_var_error(data_dir: str):
    reader = ReadCmipCtm(data_dir=data_dir)
...
def test_ReadCmipCtm_filename_from_ts_type(reader, filename, ts_type):
    assert reader._filename_from_ts_type(ts_type) == filename


def test_ReadCmipCtm_filename_from_ts_type_error(reader):
    with pytest.raises(ValueError) as e:
        reader._filename_from_ts_type("blaaa")
    assert str(e.value) == "unknown ts_type=blaaa"


def test_ReadCmipCtm_years_avail(data_dir: str):
    reader = ReadCmipCtm(data_dir=data_dir)
    assert reader.years_avail == ["2017"]


def test_ReadCmipCtm_preprocess_units():
    units = ""
    prefix = "AOD"
    assert ReadCmipCtm()._preprocess_units(units, prefix) == "1"


def test_ReadCmipCtm_open_file(data_dir: str):
    reader = ReadCmipCtm()
...
def test_ReadCmipCtm_has_var(reader, var_name, value):
    assert reader.has_var(var_name) == value


def test_ReadCmipCtm_has_var_error(reader):
    with pytest.raises(exc.VariableDefinitionError, match="input variable blaa is not supported"):
        reader.has_var("blaa")


def test_ReadCmipCtm__str__():
    assert str(ReadCmipCtm()) == "ReadCmipCtm"


def test_ReadCmipCtm__repr__():
    assert repr(ReadCmipCtm()) == "ReadCmipCtm"
...
def test_ReadCmipCtm_aux_var_defs():
    req = ReadCmipCtm.AUX_REQUIRES
    funs = ReadCmipCtm.AUX_FUNS
...
def test_read_emep_dummy_data(
    ...
    reader = ReadCmipCtm(data_dir=str(data_path / "2017"))
...
def test_read_emep_clean_filepaths(data_path: Path, year, years: list[int], freq: str):
    reader = ReadCmipCtm(data_dir=str(data_path / year))
    tst = reader.FREQ_CODES[freq]
...
def test_read_emep_clean_filepaths_error(data_path: Path, years, freq: str, error: str):
    reader = ReadCmipCtm(data_dir=str(data_path))
    tst = reader.FREQ_CODES[freq]
    filepaths = reader._filepaths
    with pytest.raises(ValueError) as e:
...
def test_emep_vars():
    ...
    reader = ReadCmipCtm(emep_vars={new_var_name: new_mapping})
...
def test_reader_regexp(tmp_path: Path):
    ...
    reader = ReadCmipCtm("test", data_dir=str(path), file_pattern=r"^file_\d\.nc$")
...
def test_reader_read_deposition():
    ...
    reader = ReadCmipCtm(data_dir=str(file_path))
