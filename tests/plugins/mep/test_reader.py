...
pytestmark = pytest.mark.xfail(
    not const.has_access_lustre, reason=f"needs access to {LUSTRE_PATH}", strict=True
)
...
def reader() -> ReadMEP:
    ...
    return ReadMEP(data_dir=str(LUSTRE_PATH))


@pytest.fixture()
def station_files(reader: ReadMEP, station: str) -> list[Path]:
