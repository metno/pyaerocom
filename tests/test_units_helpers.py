...
@pytest.mark.parametrize(
    "var_name,from_unit,to_unit,val,raises",
    [
...
@pytest.mark.parametrize(
    "from_unit,to_unit,var_name,ts_type,result,raises",
    [
        ("kg (2m)-2", "kg/m2", None, None, 0.25, does_not_raise_exception()),
        ("kg m-2", "mm", "pr", None, 1, pytest.raises(UnitConversionError)),
        ("kg m-2", "mm", "od550aer", None, None, pytest.raises(UnitConversionError)),
        ("mm", "mm d-1", "prmm", "daily", 1, does_not_raise_exception()),
        ("mm", "mm d-1", "prmm", "hourly", 24, does_not_raise_exception()),
        ("mg m-2", "ug m-2 d-1", "wetoxs", "hourly", 24e3, does_not_raise_exception()),
...
def test_get_unit_conversion_fac(from_unit, to_unit, var_name, ts_type, result, raises):
    with raises:
